...
const VolunteerMeetingsPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchVolunteerMeetings = async () => {
            try {
                const volunteerMeetingsResponse = await meetingService.getUserMeetings(currentPage, searchTerm, filter);
                setMeetings(volunteerMeetingsResponse.meetings);
                setTotalPages(volunteerMeetingsResponse.totalPages);
                setTotalItems(volunteerMeetingsResponse.totalItems);
                setIsLoading(false);
            } catch (error) {
    ...
    }, [volunteerId, currentPage, debouncedSearch, filter]);
    ...
    const handleCopyLink = (meetingId: string) => {
        const meetingUrl = `${import.meta.env.VITE_BASE_URL}/volunteer/meetings/${meetingId}`;
