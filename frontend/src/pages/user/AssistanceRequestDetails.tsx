...
const AssistanceRequestDetails: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        setIsLoading(true);
        const requestId = id || "";
        const response = await userService.fetchAssistanceRequestDetails(
          requestId
        );
  ...
  const handleSendMessage = async () => {
    if ((!messageInput.trim() && attachments.length === 0) || !request?.volunteer?.id || !id) {
        console.log("requestId: ", request?.id, "volunteerId: ", request?.volunteer?.id);
        return;
    }
    setIsSending(true);
    setSendError(null);
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <Avatar className="h-10 w-10">
                          <AvatarImage
                            src={
                              request?.volunteer?.profilePicture || profile_pic
                            }
                      ...
                      {isLoadingMessages ? (
                        ...
                        <div className="flex flex-col items-center justify-center h-full text-center">
                          ...
                          <p className="text-sm text-gray-500">
                            ...
                            {request?.volunteer?.name || "your requester"}
                          </p>
                        </div>
                      ) : (
                        <>
                      ...
                      <div className="flex items-end gap-2">
                        ...
                        <div className="flex-1">
                          <Textarea
                            ...
                            placeholder={`Message ${
                              request?.volunteer?.name || "Requester"
                            }...`}
