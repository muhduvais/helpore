...
const EditAssetModal: React.FC<EditAssetModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { isValid, errors } = validateAddAsset(
      finalFormData as AddAssetFormData
    );
    ...
    try {
      const response = await adminService.updateAsset(
        asset?.id as string,
        finalFormData
      );
      ...
      <DialogContent className="sm:max-w-[600px]">
        ...
        <form onSubmit={handleSubmit} className="grid gap-4 py-4">
          ...
          <div className="grid grid-cols-4 items-start gap-4">
            ...
            <div className="col-span-3">
              <Input
                ...
                onChange={handleInputChange}
                className={`${
                  formErrors.stocks ? "border-red-500 focus:ring-red-500" : ""
                }`}
                min="0"
              />
              {formErrors.stocks && (
          ...
          <DialogFooter>
            ...
            <Button
              type="submit"
              className={`bg-[#688D48] hover:bg-[#557239] text-white ${isImageLoading || isSubmitting ? 'cursor-not-allowed' : 'cursor-pointer'}`}
            >
              {isImageLoading ? (
                <>
