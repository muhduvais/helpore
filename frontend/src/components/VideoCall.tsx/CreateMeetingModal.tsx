...
interface CreateMeetingModalProps {
  ...
  volunteers: Array<IUserDocument>;
  ...
  useEffect(() => {
    switch (selectionMode) {
      ...
      case ParticipantSelectionMode.ALL_VOLUNTEERS:
        setSelectedParticipants(volunteers.map(volunteer => volunteer._id));
        break;
                  ...
                  <CommandGroup title="Users">
                    {users.map((user) => (
                      <CommandItem
                        key={`user-${user._id}`}
                        value={user._id}
                        onSelect={() => handleParticipantSelect(user._id)}
                      >
                        <Check
                          className={cn(
                            "mr-2 h-4 w-4",
                            selectedParticipants.includes(user._id) ? "opacity-100" : "opacity-0"
                          )}
                        />
                        {user.name} (User)
                      </CommandItem>
                  ...
                  <CommandGroup title="Volunteers">
                    {volunteers.map((volunteer) => (
                      <CommandItem
                        ...
                        value={volunteer._id}
                        onSelect={() => handleParticipantSelect(volunteer._id)}
                      >
                        <Check
                          className={cn(
                            "mr-2 h-4 w-4",
                            selectedParticipants.includes(volunteer._id) ? "opacity-100" : "opacity-0"
                          )}
                        />
          ...
          {selectedParticipants.length > 0 && (
            <div className="flex flex-wrap gap-2 mt-2">
              {selectedParticipants.map((participantId) => {
                ...
                const volunteer = volunteers.find(v => v._id === participantId);
